...
#       the 4D Local Ensemble Transform Kalman Filter for     
#       the primitive equation 1-layer model                  
#                                                            
#       This system works under the following assumptions:    
#       1. Observational errors are random with the same err  
#          cov at all time slices;                            
#       2. Obs are given at all model grid points such that   
#          H = I for all time slices. Need to change the core 
#          if obs distribution is change (module_4dletkf.f90; 
#       3. The adaptive inflation/localization is not used;   
#       4. Model is perfect.                                  
#       5. The total integration < 100 days. This is because  
#          all day index is limited at 2 digits, which is     
#          merely because that is enough for testing. Need to 
#          modify code and script as 0000:00:00 if longer     
#          integration is needed.    
#                         
# HIST: - Jan 31, 2010: created by CK
#       - Apr 16, 2022: modified by CK for DA class
#                                                            
# AUTH: Chanh Kieu, Indiana University (C)                       
#                                                            
#=====================================================================
main_dir="/N/slate/ckieu/da_4dletkf_swe"
ninterval=3                            # restart interval [h]
nensemble=30                           # number of ensemble members  
restart="Y"                            # restart/continuous run opts 
nt=73                                  # forecast length [h] 
ncycle=$(($nt/$ninterval+1))           # number of assimilation cycles
cold_start="T"                         # cold start option
history="L"                            # back up history option 
t_window=3600                          # assimilation window length [s]
dt_window=600                          # time between obs slices each window [s] 
mpi_run="Y"                            # option for MPI run 
nslices=$(($t_window/$dt_window+1))    # number of time slices each window
letkf_flag="N"                         # option for pure LETKF
if [ "${letkf_flag}" == "Y" ]; then
    t_window=3600
    dt_window=3600
    nslices=2
    nslices1=1
    ...
    if [ "$t_window" -ge "$(($ninterval*3600))" ]; then
        echo "DA window has to be smaller than DA cycle...exit"
...
echo " Number of DA cycles that the script will run is : $ncycle"
...
echo " LETKF flag for controling time slice            : $letkf_flag"
echo " Number of time slices for assimilation          : $nslices1"
echo "==========================================================="
#
...
zscale       = 800e+3          ! scale of z-anomaly               [m]
icen         = 65              ! i-center of the anomaly          [] 
jcen         = 7               ! j-center of the anomaly          []
obs_err_u    = 1.0             ! obs error for u                  [m/s]
obs_err_v    = 1.0             ! obs error for v                  [m/s]
obs_err_z    = 3.0             ! obs error for z                  [m] 
bgd_err_u    = 5.0             ! background error for u           [m]
bgd_err_v    = 5.0             ! background error for v           [m/s]
bgd_err_z    = 18.             ! background error for z           [m]
...
if [ "$restart" = "Y" ]; then
    ...
    cd ${main_dir}/truth
    ln -sf ${main_dir}/run/namelist.swe ./
...
#
echo "Running a control simulation now ..."
cd ${main_dir}/ctl/
ln -sf ${main_dir}/run/namelist.swe ./
